...
export class AdicionarInoculacaoPage implements OnInit {
  ...
  public salvarInoculacao() {
    ...
    this.inoculacao.dataInoculacao = Date.parse(this.dataInoculacao) + this.umDia;

    // persiste Inoculacao no banoc de dados, depois persiste trinta DiaObservacao
    this.inoculcaoService.salvarInoculacao(this.inoculacao).subscribe((data) => {
      console.log('salvo');
      let dataObservacao = this.inoculacao.dataInoculacao;
      for (let i = 0; i < 30; i++) {

        dataObservacao = dataObservacao + this.umDia;
